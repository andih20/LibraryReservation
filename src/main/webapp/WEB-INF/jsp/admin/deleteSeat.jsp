...
<html>
<head>
    <title>deleteSeat</title>
    <script type="text/javascript">
        function SeatDel(id){
            if(window.confirm("是否删除该座位？")){
                window.location.href = "/LibraryReservation_war_exploded/adminSeat/deleteSeat?id="+id;
            }
        }
...
<c:if test="${AllSeat.size()!=0}">
    ...
    <form:form action="${pageContext.request.contextPath}/adminSeat/deleteSeat" modelAttribute="Seat" method="post">
        <table>
            ...
            <c:if test="${SelectSeatInfoByFloor.size()!=0}">
                ...
                <c:forEach items="${SelectFloorInfo}" var="seat">
                    <tr>
                        ...
                        <td>${seat.impair}</td>
                        <td>${seat.floor}</td>
                        <td><a href="javascript:SeatDel('${seat.id}')">删除</a></td>
                    </tr>
                </c:forEach>
                ...
                <tr>
                    <td colspan="5" align="right">
                        ...
                        第${pageCur}页&nbsp;&nbsp;
                        <c:url var="url_pre" value="/adminSeat/deleteSeat">
                            <c:param name="pageCur" value="${pageCur - 1 }"/>
                        </c:url>
                        <c:url var="url_next" value="/adminSeat/deleteSeat">
                            <c:param name="pageCur" value="${pageCur + 1 }"/>
                        </c:url>
                        <!-- 第一页没有上一页 -->
                        <c:if test="${pageCur != 1 }">
